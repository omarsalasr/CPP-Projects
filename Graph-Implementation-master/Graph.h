...
class graph {
      ...
      while(!q.empty()) {
        ...
        for(edge &e : vertices[u].outgoing) {
          ...
          if(report[v].state == UNDISCOVERED) {
            report[v].npaths += report[u].npaths;//add the pred npaths
            ...
            if(report[u].dist+1 == report[v].dist){//more than 1 path with the same dist
  ...
  public:
    ...
    bool extract_path(const vector<vertex_label> & rpt, 
      ...
      if(rpt.size() != num_nodes())
        return false;
      if(rpt[dest].dist != -1)//reachable vertices only
        _extract_path(rpt,dest,path);
      reverse(path.begin(),path.end());//reverse the order O(|V|)
      return true;
    }
  ...
  private:
    ...
    void _extract_path(const vector<vertex_label> & rpt, int dest, vector<int> & path){
      path.push_back(dest);//add to the path
      if(rpt[dest].pred == dest)//reached the top
        return;
      dest = rpt[dest].pred;
      _extract_path(rpt,dest,path);
    }
  ...
  public:
    ...
    bool dag_critical_paths(vector<vertex_label> & rpt) {
      ...
      for(i = 0; i < rpt.size(); i++){//initialize the fields
        rpt[i].state = UNDISCOVERED;
        rpt[i].dist = 0.0;
        rpt[i].pred = -1;
      }
      for(i = 0; i < num_nodes(); i++){//go through each vertex that hasn't been discovered
  ...
  double _dag_critical_paths(vector<vertex_label> & rpt, int id){
    if(rpt[id].state == DISCOVERED){//visited has a distance already
      return rpt[id].dist;
    }
    if(vertices[id].incoming.size() == 0){//vertex with indegree 0
      rpt[id].dist = 0;
      rpt[id].pred = id;
      rpt[id].state = DISCOVERED;
      return 0.0;
    }
    int i;
    double temp = 0.0;
    double longest = 0.0;
    for(edge &e : vertices[id].incoming){//go through the incoming edges
      i = e.vertex_id;
      temp = e.weight + _dag_critical_paths(rpt,i);//get the total weight
      rpt[i].state = DISCOVERED;
      if(temp >= longest){//check which weight is the biggest
        longest = temp;
        rpt[id].pred = i;
      }
    }
  ...
  public:
    ...
    bool dag_num_paths(vector<vertex_label> & rpt) {
      ...
      int in, out;
      for(i = 0; i < num_nodes(); i++){//go through each vertex
        in = _dag_num_paths_in(rpt,i);//number of input paths
        out = _dag_num_paths_out(rpt,i);//number of output paths
        rpt[i].npaths = in*out;//number of io paths
      }
      return true;
    }
  ...
  private:
    ...
    int _dag_num_paths_in(vector<vertex_label> & rpt, int id){
      if(vertices[id].incoming.size() == 0)
        return 1;
      int sum = 0;
      for(edge &e : vertices[id].incoming)//sum up all the input paths
        sum += _dag_num_paths_in(rpt, e.vertex_id);
      return sum;
    }

    int _dag_num_paths_out(vector<vertex_label> & rpt, int id){
      if(vertices[id].outgoing.size() == 0)
        return 1;
      int sum = 0;
      for(edge &e : vertices[id].outgoing)//sum up all the output paths
        sum += _dag_num_paths_out(rpt, e.vertex_id);
      return sum;
    }
  ...
  public:
    ...
    bool valid_topo_order(const vector<int> & order) {
      if(has_cycle())
        return false;
      int i,v;
      vector<int> vis;//vector to keep track of who has been visited
      for(i = 0; i < order.size(); i++){
        vis.push_back(0);
      }
      for(i = 0; i < order.size(); i++){
        if(vertices[order[i]].incoming.size() == 0){//top of the dag
          vis[vertices[order[i]].id] = 1;
          continue;
        }
        for(edge &e : vertices[order[i]].incoming){//examine the incoming edges
          v = e.vertex_id;
          if(vis[vertices[v].id] == 0){//check if the pred hasn't been visited
            return false;
          }
        }
        vis[vertices[order[i]].id] = 1;
    ...
    bool enum_paths(int target, vector<string> &paths) {
      ...
      if(has_cycle() || target < 0 || target >= num_nodes())
        return false;
      int i;
      vector<int> pth;//keep track of the path in terms of id's
      for(i = 0; i < num_nodes(); i++)//go through each vertex
        if(vertices[i].incoming.size() == 0)//find the vertex with indegree 0
          _enum_paths(target, paths,i, pth);
      return true;
    }

  private:

    void _enum_paths(int target, vector<string> &paths, int id, vector<int> pth){
      if(vertices[id].outgoing.size() == 0)//reached an output edge
        return;
      pth.push_back(id);
      if(id == target){//reached my target
        paths.push_back(id_vec2string(pth));//add the path in string format
        return;
      }
      int i;
      for(edge &e : vertices[id].outgoing)//go through each outgoing edge
        _enum_paths(target,paths,e.vertex_id,pth);
    }
